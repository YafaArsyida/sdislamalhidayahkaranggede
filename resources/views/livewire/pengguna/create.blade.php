...
<div>
    <div wire:ignore.self class="modal fade" id="ModalAddPengguna" tabindex="-1" aria-labelledby="ModalAddPengguna" aria-hidden="true">
                                ...
                                <div class="form-check">
                                    @foreach ($select_jenjang as $item)
                                        <input 
                                            ...
                                            type="checkbox" 
                                            id="ms_jenjang_id_{{ $item->ms_jenjang_id }}" 
                                            wire:model="ms_jenjang_id" 
                                            value="{{ $item->ms_jenjang_id }}" 
                                        >
                                        <label class="form-check-label" for="ms_jenjang_id_{{ $item->ms_jenjang_id }}">
                                            {{ $item->nama_jenjang }}
                                        </label>
                                        <br>
                                    @endforeach
